...
void subdivideLoop(Mesh* inputMesh, Mesh* subdivMesh) {
  ...
  // The original method did not set next and prev for the boundary edges.
  HalfEdge *next;
  for (HalfEdge &edge : subdivMesh->HalfEdges){

      if (!edge.polygon){ // Edge is a boundary edge
          next = edge.twin;
          // Keep going until you find next halfedge on the boundary
          while (next->polygon){
            next = next->prev->twin;
          }
          edge.next = next;   // Update next
          next->prev = &edge; // Update prev
      }
  }
