...
void MainWindow::on_ShowRefl_toggled(bool checked){
    ui->MainDisplay->showRefl = checked;
    ui->MainDisplay->updateMatrices();
}

void MainWindow::on_refDensity_valueChanged(double density)
{
    qDebug() << density;
    ui->MainDisplay->rDensity = density;
    ui->MainDisplay->updateMatrices();
}
